import { join, basename, resolve, extname, posix } from 'node:path';
...
async function externalizeScript(html, assets) {
	...
	const updated = html.replace(
		/<script>(\n\s*\{\n\s*__sveltekit_(.{4,16}?)\s[\s\S]+)<\/script>/,
		(_, content) => {
			innerText = content;
			hash = Buffer.from(hash_script(content), 'base64').toString('hex').slice(0, 7);
			return `<script src="script-${hash}.js"></script>`;
		}
	);
	const externalized_scriptsPath = join(assets, `script-${hash}.js`);
...
function extractorFactory(builder, buildDir) {
	const extractTo = (targetDir) => {
		...
		return async (sourcePath, tsConfig = {}) => {
			...
			return posix.join(targetDir, targetFileName);
		};
	};
